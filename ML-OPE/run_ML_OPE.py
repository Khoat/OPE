...
def main():
    # Check input
    if len(sys.argv) != 5:
        print"usage: python run_ML_OPE.py [train file] [setting file] [model folder] [divided data folder]"
        exit()
    ...
    while i < ddict['iter_train']:
        ...
        while True:
            ...
            (LD2, ld2_list) = utilities.compute_perplexities_vb(ml_ope.beta, ddict['alpha'], ddict['eta'], ddict['iter_infer'],\
                                                             corpusids_part1, corpuscts_part1, corpusids_part2, corpuscts_part2)
            # Search top words of each topics
            list_tops = utilities.list_top(ml_ope.beta, tops)
            # Write files
            utilities.write_file(i, j, ml_ope.beta, time_e, time_m, theta, sparsity, LD2, ld2_list, list_tops, tops, model_folder)
        datafp.close()
