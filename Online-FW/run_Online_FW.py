...
def main():
    # Check input
    if len(sys.argv) != 5:
        print"usage: python run_Online_FW.py [train file] [setting file] [model folder] [divided data folder]"
        exit()
    ...
    while i < ddict['iter_train']:
        ...
        while True:
            ...
            (LD2, ld2_list) = utilities.compute_perplexities_fw(online_fw._lambda, ddict['iter_infer'], \
                                      corpusids_part1, corpuscts_part1, corpusids_part2, corpuscts_part2)   
            # Search top words of each topics
            list_tops = utilities.list_top(online_fw._lambda, tops)    
            # Write files
            utilities.write_file(i, j, online_fw._lambda, time_e, time_m, theta, sparsity, LD2, ld2_list, list_tops, tops, model_folder)
        datafp.close()
